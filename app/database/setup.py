...
def create_tables(app,db, config_class):
    ...
    try:
        with app.app_context():  # me permite sicronizar la base de datos con la aplicación
            ...
            db.metadata.create_all(app=app)    # crea la tabla en la base de datos que se encuentra en la cadena de conexion(url)                
            logging.debug(db.metadata.create_all(app=app))
